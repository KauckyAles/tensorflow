...
class TpuEmbeddingV3CPUOpsTest(parameterized.TestCase, test.TestCase):
  ...
  def test_invalid_id_counts_with_convert_to_sparse_core_csr_wrapped_coo_tensor(self):
    sample_count = 16
    token_count = 1024
    combiner = "sum"
    sparse_feature = sparse_ops.sparse_reorder(
    ...
    for i in range(4):
      ...
      invalid_id_counts = array_ops.concat([id_counts_before, new_last_element], axis=0)
      id_counts_list.append(invalid_id_counts)

    with self.assertRaisesRegex(Exception, "should not exceed the size of the corresponding sorted tensors"):
      xla_ops.convert_to_sparse_core_csr_wrapped_coo_tensor(
          sorted_row_ids_list=sorted_row_ids_list,
          sorted_col_ids_list=sorted_col_ids_list,
          sorted_gains_list=sorted_gains_list,
