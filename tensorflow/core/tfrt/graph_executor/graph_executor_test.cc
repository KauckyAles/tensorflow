...
TEST_F(GraphExecutorTest, CreateRequestInfoPopulatesRpcOptions) {
  GraphDef graph_def;
  TF_ASSERT_OK(GetSimpleGraphDef(graph_def));
  ...
  run_options.is_rpc_cancelled = [&is_cancelled]() { return is_cancelled; };
  ...
  EXPECT_EQ(
      ...
      now);
  EXPECT_FALSE(fallback_request_state->is_rpc_cancelled()());
  is_cancelled = true;
  EXPECT_TRUE(fallback_request_state->is_rpc_cancelled()());
}
