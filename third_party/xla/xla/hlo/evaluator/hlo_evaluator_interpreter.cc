...
LinearizedInterpreter::Build(const HloInstruction* instruction,
  ...
  if (computation) {
    ...
    for (const HloInstruction* instr : post_order) {
      ...
      current_offset = xla::RoundUpTo(current_offset,
                                      AlignmentOfPrimitiveType(promoted_type));
...
absl::Status LinearizedInterpreter::InitializeAccumulators(
  ...
  for (int i = 0; i < num_args; ++i) {
    ...
    PrimitiveType type =
        computation_->parameter_instruction(i)->shape().element_type();
    primitive_util::PrimitiveTypeSwitch<void>(
        [&](auto type_tag) {
          constexpr PrimitiveType kType = decltype(type_tag)::value;
          if constexpr (primitive_util::IsArrayType(kType)) {
            using T = primitive_util::NativeTypeOf<kType>;
...
absl::Status LinearizedInterpreter::EvaluateReductionBatchStep(
  ...
  auto& saved_accumulators = scratchpad.saved_accumulators_;
      ...
      primitive_util::PrimitiveTypeSwitch<void>(
          [&](auto type_tag) {
            ...
            if constexpr (primitive_util::IsArrayType(kType)) {
              ...
              for (int b = 0; b < batch_size_; ++b) {
                if ((out_of_bounds_mask >> b) & 1) {
                  std::memcpy(dest_base + b * elem_size,
                              src_base + b * elem_size, elem_size);
                }
              }
            }
          },
...
void LinearizedInterpreter::PopulateOutputIndicesForReduce(
    Scratchpad& scratchpad, const Shape& shape, int64_t start_elem,
    int actual_batch_size, const ReduceState& state) const {
  auto output_index =
      IndexUtil::LinearIndexToMultidimensionalIndex(shape, start_elem);
  for (int b = 0; b < batch_size_; ++b) {
    if (b < actual_batch_size) {
      for (int i = 0; i < state.reduction_inputs.size(); ++i) {
      ...
      if (b + 1 < actual_batch_size) {
        IncrementIndex(shape, absl::MakeSpan(output_index));
      }
    } else {
      for (int i = 0; i < state.reduction_inputs.size(); ++i) {
        int arg_rank = state.reduction_inputs[i].shape->dimensions().size();
...
absl::Status LinearizedInterpreter::RunReductionLoop(
    Scratchpad& scratchpad, const ReduceState& state,
...
absl::Status LinearizedInterpreter::EvaluateReduceBatch(
  ...
  auto& temp_results = scratchpad.temp_results_;
  ...
  PopulateOutputIndicesForReduce(scratchpad, results[0].shape(), start_elem,
                                 actual_batch_size, state);

  TF_RETURN_IF_ERROR(RunReductionLoop(scratchpad, state, actual_batch_size));
...
void LinearizedInterpreter::PopulateOutputIndicesForReduceWindow(
    Scratchpad& scratchpad, const Shape& shape, int64_t start_elem,
    int actual_batch_size) const {
  int rank = shape.dimensions().size();
  auto output_index =
  ...
  for (int b = 0; b < actual_batch_size; ++b) {
    for (int d = 0; d < rank; ++d) {
      scratchpad.output_indices_[b * rank + d] = output_index[d];
    }
    if (b + 1 < actual_batch_size) {
      IncrementIndex(shape, absl::MakeSpan(output_index));
    }
...
void LinearizedInterpreter::PopulateBaseIndexInvariants(
    Scratchpad& scratchpad, const ReduceWindowState& state,
    int actual_batch_size) const {
  int rank = state.config.window_dims.size();
  for (size_t d = 0; d < rank; ++d) {
    int64_t stride = state.config.window_dims[d].stride;
    int64_t padding_low = state.config.window_dims[d].padding_low;
    for (int b = 0; b < actual_batch_size; ++b) {
      scratchpad.base_index_invariants_[d * batch_size_ + b] =
          scratchpad.output_indices_[b * rank + d] * stride - padding_low;
    }
  }
}

bool LinearizedInterpreter::CheckIsInterior(const Scratchpad& scratchpad,
                                            const ReduceWindowState& state,
                                            int actual_batch_size) const {
  int rank = state.config.window_dims.size();
...
absl::Status LinearizedInterpreter::RunReduceWindowLoop(
    Scratchpad& scratchpad, const ReduceWindowState& state,
    int actual_batch_size, bool is_interior) const {
  int rank = state.config.window_dims.size();
  uint16_t valid_batch_mask = ((1U << actual_batch_size) - 1);
  DimensionVector window_index(rank, 0);
  absl::Span<int64_t> window_index_span = absl::MakeSpan(window_index);
  int64_t* base_indices_ptr = scratchpad.base_indices_.data();
  const int64_t* invariants_ptr = scratchpad.base_index_invariants_.data();
...
absl::Status LinearizedInterpreter::EvaluateReduceWindowBatch(
  ...
  // Reusable buffers from scratchpad
  auto& scratchpad = state.scratchpads[thread_id + 1];
  auto& base_index_invariants = scratchpad.base_index_invariants_;
  auto& base_indices = scratchpad.base_indices_;
  auto& output_indices = scratchpad.output_indices_;
  auto& temp_results = scratchpad.temp_results_;
  ...
  PopulateOutputIndicesForReduceWindow(scratchpad, results[0].shape(),
                                       start_elem, actual_batch_size);

  PopulateBaseIndexInvariants(scratchpad, state, actual_batch_size);

  bool is_interior = CheckIsInterior(scratchpad, state, actual_batch_size);

  TF_RETURN_IF_ERROR(
      RunReduceWindowLoop(scratchpad, state, actual_batch_size, is_interior));
