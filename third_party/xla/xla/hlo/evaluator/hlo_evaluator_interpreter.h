...
class LinearizedInterpreter {
 public:
  ...
  class Scratchpad {
   ...
   private:
    ...
    std::vector<const void*> args_ptrs_;
    std::vector<void*> results_ptrs_;
    std::vector<std::vector<char>> saved_accumulators_;

    std::vector<std::vector<int64_t>> batched_linear_indices_;
    std::vector<std::vector<char>> batch_deferred_buffers_;
    std::vector<int64_t> base_index_invariants_;
    std::vector<int64_t> base_indices_;
    std::vector<int64_t> output_indices_;
    std::vector<std::vector<char>> temp_results_;
 ...
 private:
  ...
  void PopulateOutputIndicesForReduce(Scratchpad& scratchpad,
                                      const Shape& shape, int64_t start_elem,
                                      int actual_batch_size,
                                      const ReduceState& state) const;

  absl::Status RunReductionLoop(Scratchpad& scratchpad,
                                const ReduceState& state,
                                int actual_batch_size) const;

  void PopulateOutputIndicesForReduceWindow(Scratchpad& scratchpad,
                                            const Shape& shape,
                                            int64_t start_elem,
                                            int actual_batch_size) const;

  void PopulateBaseIndexInvariants(Scratchpad& scratchpad,
                                   const ReduceWindowState& state,
                                   int actual_batch_size) const;

  bool CheckIsInterior(const Scratchpad& scratchpad,
                       const ReduceWindowState& state,
                       int actual_batch_size) const;

  absl::Status RunReduceWindowLoop(Scratchpad& scratchpad,
                                   const ReduceWindowState& state,
                                   int actual_batch_size,
