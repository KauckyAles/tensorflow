...
absl::StatusOr<bool> DevicelessTableHasDataForSort(
  ...
  return lookup.CanLookup(device_description.cub_version(),
                          device_description.name(), key_type_size,
...
absl::StatusOr<bool> ShouldRewriteSort(
  ...
  if (sort == nullptr || !IsCubCompatibleSort(device_description, sort)) {
    return false;
  }

  if (!is_deviceless || early_exit_with_layouts) {
    return true;
  }
  ...
  ASSIGN_OR_RETURN(bool theres_deviceless_data,
                   DevicelessTableHasDataForSort(device_description, *sort));
  if (theres_deviceless_data) {
    return true;
  }

  if (deviceless_cub_mode == DebugOptions::DEVICELESS_CUB_FAIL_CLOSED) {
    return absl::NotFoundError(
        absl::StrFormat("Missing deviceless CUB scratch size data for sort"));
  }

  TF_RET_CHECK(deviceless_cub_mode == DebugOptions::DEVICELESS_CUB_FAIL_OPEN);
  return false;
}
  ...
  for (auto* inst : computation->instructions()) {
    ...
    ASSIGN_OR_RETURN(
        ...
        ShouldRewriteSort(device_description_, sort, is_deviceless_,
                          early_exit_with_layouts_, deviceless_cub_mode));
    if (should_rewrite) {
...
absl::StatusOr<bool> SortRewriter::RunImpl(
  ...
  if (is_deviceless_ && !early_exit_with_layouts_ &&
